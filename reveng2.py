...
def make_forward_gp_projections(stat_df, year=2024):
    yr4_model = build_model()
    ...
    #     print(yr4_group[index], min(proj_y_4[index][0] + statistics.mean(statline[-4:]), 82))

    # yr4_stat_list_scaled = X_4_scaler.transform(yr4_stat_list)
    # example_batch = yr4_stat_list_scaled[:10]
    # example_result = yr4_model.predict(example_batch)
    # print(example_result)
    # for i in range(len(example_result)):
    #     print(yr4_group[i], example_result[i])
    ...
    for index, statline in enumerate(yr4_stat_list):
        ...
        for sim in range(sim_count):
            prediction.append(min(predictions[sim][index][0] + statistics.mean(statline[-4:]), 82))
        print(yr4_group[index], prediction)


stat_df = scrape_player_statistics(True)
make_forward_gp_projections(stat_df)