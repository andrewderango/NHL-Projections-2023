        ...
        while response == '':
            try:
                ...
                for index, row in temp_df.iterrows():
                    ...
                    stat_df.loc[row['Player'].replace('ä', 'a'), f'{year+1} {situation_reassignment[situation]} A1/60'] = round(float(row['First Assists'])/float(row['TOI']),4)
                    stat_df.loc[row['Player'].replace('ä', 'a'), f'{year+1} {situation_reassignment[situation]} A2/60'] = round(float(row['Second Assists'])/float(row['TOI']),4)
                    stat_df.loc[row['Player'].replace('ä', 'a'), f'{year+1} {situation_reassignment[situation]} ixG/60'] = round(float(row['ixG'])/float(row['TOI']),4)
                    stat_df.loc[row['Player'].replace('ä', 'a'), f'{year+1} {situation_reassignment[situation]} Shots/60'] = round(float(row['Shots'])/float(row['TOI']),4)
